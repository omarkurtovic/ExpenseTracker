...
namespace ExpenseTrackerWebApi.Features.Dashboard.Handlers
{
    public class GetDashboardSummaryQueryHandler : IRequestHandler<GetDashboardSummaryQuery, DashboardSummaryDto>
    {
        ...
        public async Task<DashboardSummaryDto> Handle(GetDashboardSummaryQuery request, CancellationToken cancellationToken)
        {
            ...
            result.CumulativeIncomePerMonth = GetCumulativePerMonth(result.Transactions, TransactionTypeDto.Income, request.MonthsBehindToConsider);

            return result;
        }


        private List<CategoryData> GetTopExpenseCategories(List<TransactionDto> transactions, int monthsBehindToConsider = 5, int maxCategoriesToShow = 6)
        {
            ...
            .Where(t => t.CategoryDto!.Type == TransactionTypeDto.Expense)
            .Where(t => t.Date >= dateStart)
            .GroupBy(t => t.CategoryDto)
        ...
        private List<CumulativeData> GetCumulativePerMonth(List<TransactionDto> transactions, TransactionTypeDto type, int monthsBehindToConsider = 5)
        {
            ...
            .Where(t => t.CategoryDto!.Type == type)
            .Where(t => t.Date >= dateStart)
            ...
            .Select(g => new
            {
                Year = g.Key.Year,
                Month = g.Key.Month,
                Amount = g.Sum(t => type == TransactionTypeDto.Expense ? -t.Amount : t.Amount)
            })
            ...
            foreach (var monthData in monthlyExpenses)
            {
                foreach(var data in result)
                {
                    ...
                    {
                        data.Amount = (decimal)monthData.Amount!;
                        break;
                    }
                }
            }
